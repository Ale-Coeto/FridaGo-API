...
# Load and compile the age and gender
...
def preprocess_age_gender(face_image, input_size):
    resized_face = cv2.resize(face_image, (input_size[0], input_size[1]))
    resized_face = resized_face.astype(np.float32)  # Convert to float32
    resized_face = resized_face.transpose((2, 0, 1))  # Change to CHW format
    resized_face = resized_face[None, :]  # Add batch dimension
...
for (xmin, ymin, xmax, ymax) in detected_faces:
    ...
    preprocessed_age_gender = preprocess_age_gender(face_region, age_gender_size)
    # Convert NumPy array to OpenVINO Tensor
    emotion_tensor = Tensor(preprocessed_face)
    age_gender_tensor = Tensor(preprocessed_age_gender)
    ...
    # Age gender recognition inference
    age_infer_request = age_gender_compiled_model.create_infer_request()
    age_infer_request.set_input_tensor(age_gender_tensor)
    age_infer_request.infer()
    age_gender_output = age_infer_request.get_output_tensor().data
    # print(age_gender_output)
